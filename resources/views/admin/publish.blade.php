...
<div class="row">
    <div class="col-md-8 col-md-offset-2">
        <form action="" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            <input type="hidden" name="folder" value="{{ isset($product)?(isset($product['product']->folder) ? $product['product']->folder : '0'):'0'}}">
            <div class="card">
                <div class="card-body"> 
                    ...
                    <hr>
                    @foreach ($locales as $locale)
                    @php $lKey = false;if(isset($product)) {if($product['translations'] != null) { $lKey = array_search($locale, array_column($product['translations'], 'locale')); }} @endphp
                    ...
                    <div class="locale-container locale-container-{{$locale}}" @if ($currentLocale == $locale) style="display:block;" @endif>
                         <div class="md-form">
                            ...
                            <input type="text" name="name[]" value="{{ $lKey !== false ? (isset($product)?$product['translations'][$lKey]->name : ''):'' }}" id="publishForm-name-{{$locale}}" class="form-control">
                            <label for="publishForm-name-{{$locale}}">{{__('admin_pages.product_name')}}({{$locale}})</label>
                        </div>  
                        <div class="md-form">
                            <i class="fa fa-pencil prefix grey-text"></i>
                            <textarea name="description[]" type="text" id="productDescr-{{$locale}}" class="md-textarea">{{ $lKey != false ? (isset($product)?$product['translations'][$lKey]->description : ''):'' }}</textarea>
                            <label for="productDescr-{{$locale}}">{{__('admin_pages.product_description')}}({{$locale}})</label>
                        </div>
                        <div class="md-form">
                            <i class="fa fa-eur prefix grey-text"></i>
                            <input type="text" name="price[]" value="{{ $lKey !== false ?  (isset($product)?$product['translations'][$lKey]->price : ''):'' }}" id="publishForm-price-{{$locale}}" class="form-control">
                    ...
                    <div class="md-form">
                        ...
                        <input type="text" name="quantity" value="{{ isset($product)?(isset($product['product']->quantity) ? $product['product']->quantity : ''):''}}" id="publishForm-quantity" class="form-control">
                        <label for="publishForm-quantity">{{__('admin_pages.quantity')}}</label>
                    </div>
                    <div class="md-form">
                        <i class="fa fa-sort prefix grey-text"></i>
                        <input type="text" name="order_position" value="{{ isset($product)?(isset($product['product']->order_position) ? $product['product']->order_position : ''):''}}" id="publishForm-position" class="form-control">
                        <label for="publishForm-position">{{__('admin_pages.order_position')}}</label>
                    </div>
                    <div class="md-form">
                        <i class="fa fa-link prefix grey-text"></i>
                        <input type="text" name="link_to" value="{{ isset($product)?(isset($product['product']->link_to) ? $product['product']->link_to : ''):''}}" id="publishForm-linkto" class="form-control">
                    ...
                    <div class="md-form">
                        ...
                        <div class="element-label-text bordered-div">
                            <select class="selectpicker" name="category_id" data-style="btn-secondary"> 
                                ...
                                <option value="{{$aCateg->id}}"> {{ isset($product)?(isset($product['product']->category_id) && $product['product']->category_id == $aCateg->id ? 'selected' : ''):''}}>{{$aCateg->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="md-form">
                        ...
                        <div class="element-label-text bordered-div">
                            <input type="checkbox" class="switch-me {{ isset($product)?(isset($product->hidden) && $product['product']->hidden == 1 ? 'checked' : ''):''}}" data-on-color="secondary" name="hidden"> 
                        </div>
                    </div>
                    <div class="md-form">
                        <label class="alone">{{__('admin_pages.tags_product')}}</label>
                        <div class="element-label-text bordered-div">
                            <input type="text" data-role="tagsinput" value="{{ isset($product)?(isset($product['product']->tags) ? $product['product']->tags : ''):''}}" name="tags"  class="form-control input-tags"> 
                        </div>
                    </div>
                    ...
                    <div class="md-form clone-file-upload">
                        ...
                        <div class="element-label-text">
                            <div class="upload-wrap">
                                <button type="button" class="btn btn-secondary">{{ isset($product)?(isset($product['product']->image) ? __('admin_pages.choose_new_cover_img') : __('admin_pages.choose_cover_img')):''}}</button>
